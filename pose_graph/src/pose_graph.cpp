...
void PoseGraph::registerPub(ros::NodeHandle &n)
{
    ...
    pub_odom = n.advertise<nav_msgs::Odometry>("loop_odometry", 1000);
    pub_pcl_update = n.advertise<sensor_msgs::PointCloud>("loop_update", 1000);
}
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    else
    {
  	...
  	if (loop_index != -1)
  	{
        ...
        {
            cout<< "addKeyFrame               keyframelist size: "<< keyframelist.size()<< "  cur_kf index:"<<cur_kf->index << endl;
    ...
    nav_msgs::Odometry odom_stamped;
    odom_stamped.header = pose_stamped.header;
    odom_stamped.pose.pose = pose_stamped.pose;
    pub_odom.publish(odom_stamped);
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            cout<< "optimize4DoF               keyframelist size: "<< keyframelist.size()<< "  cur_kf index:"<<cur_kf->index <<"   "<< cur_index<< endl;
...
{
    sensor_msgs::PointCloud pcl_update;
    sensor_msgs::ChannelFloat32 q_w;
    sensor_msgs::ChannelFloat32 q_x;
    sensor_msgs::ChannelFloat32 q_y;
    sensor_msgs::ChannelFloat32 q_z;
    m_keyframelist.lock();
    list<KeyFrame*>::iterator it;
...
//        printf("path p: %f, %f, %f\n",  P.x(),  P.z(),  P.y() );
        ...
        p.y = P.y();
        p.z = P.z();
        pcl_update.points.push_back(p);
        q_w.values.push_back(Q.w());
        q_x.values.push_back(Q.x());
        q_y.values.push_back(Q.y());
        q_z.values.push_back(Q.z());
        pcl_update.header = pose_stamped.header;
    ...
    pcl_update.channels.push_back(q_w);
    pcl_update.channels.push_back(q_x);
    pcl_update.channels.push_back(q_y);
    pcl_update.channels.push_back(q_z);
    publish();
    m_keyframelist.unlock();
    pub_pcl_update.publish(pcl_update);
}
