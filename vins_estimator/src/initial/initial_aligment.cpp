...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        tmp_A.block<3, 1>(0, 9) = frame_i->second.R.transpose() * (frame_j->second.T - frame_i->second.T) / 100.0;   //shan:对应到scale
    ...
    ROS_ERROR("estimated scale: %f", s);
    g = x.segment<3>(n_state - 4);
    ROS_ERROR_STREAM(" result g     " << g.norm() << " " << g.transpose());
    ...
    s = (x.tail<1>())(0) / 100.0;
    (x.tail<1>())(0) = s;
    ROS_ERROR_STREAM(" refine     " << g.norm() << " " << g.transpose());
    if(s < 0.0 )
        return false;
    else
        return true;
...
{
    int all_frame_count = all_image_frame.size();
    int n_state = all_frame_count * 3 + 3;//shan: no scale now
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        MatrixXd tmp_A(6, 9);//shan: no scale now
        tmp_A.setZero();
        VectorXd tmp_b(6);
        tmp_b.setZero();
